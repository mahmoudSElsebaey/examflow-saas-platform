...
export function OrgSettingsPage() {
  ...
  }, [accessToken, orgId, reset, t, toast])
  ...
  const onLeave = async () => {
    if (!accessToken || !orgId) return
    if (!window.confirm(t('org.confirmLeave', { defaultValue: 'Leave this organization?' })))
      return
    setLeaving(true)
    setError(null)
    try {
  ...
  const onTransfer = async (membershipId: string) => {
    ...
    if (
      !window.confirm(
        t('org.confirmTransfer', { defaultValue: 'Transfer ownership to this member?' })
      )
    )
      return
              ...
              <CardContent>
                <form onSubmit={handleSubmit(onSave)} className="space-y-4" noValidate>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <input
                        ...
                        value={
                          watch('primaryColor')?.match(/^#[0-9A-Fa-f]{6}$/)
                            ? watch('primaryColor')!
                            : '#0f766e'
                        }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="logoUrl"
                      placeholder={t('whiteLabel.logoPlaceholder')}
                      {...register('logoUrl')}
                    />
                    <input
                      type="file"
                    ...
                    {logoPreview && logoPreview.startsWith('data:image') && (
                      <img
                        src={logoPreview}
                        alt={t('whiteLabel.logoPreview')}
                        className="mt-2 h-12 w-12 rounded-lg border border-border object-contain"
                      />
                    )}
                  </div>
                  ...
                  {saved && (
                    <p className="text-center text-xs text-success">{t('whiteLabel.saved')}</p>
                  )}
                </form>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/60">
              <CardHeader>
                ...
                <CardDescription>
                  {t('org.transferHint', {
                    defaultValue:
                      'Choose an active member to become the new owner. You will become admin.',
                  })}
                </CardDescription>
              </CardHeader>
            ...
            <Card className="border-border/60">
              ...
              <CardContent>
                ...
                >
                  {leaving
                    ? t('common.loading')
                    : t('org.leave', { defaultValue: 'Leave organization' })}
                </Button>
              </CardContent>
            </Card>
          )}
