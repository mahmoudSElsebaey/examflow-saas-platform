import { appConfig } from '@/config/app'
import type { ApiResponse } from '@/features/auth/types'

export type NotificationItem = {
  id: string
  type: string
  title: string
  body: string
  link?: string | null
  readAt?: string | null
  createdAt: string
}

const BASE = `${appConfig.API_BASE_URL}/notifications`

async function request<T>(path: string, token: string, options: RequestInit = {}) {
  const res = await fetch(`${BASE}${path}`, {
    ...options,
    credentials: 'include',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
      ...(options.headers || {}),
    },
  })
  const json = (await res.json()) as ApiResponse<T>
  if (!res.ok) throw new Error(json.message || 'Request failed')
  return json
}

export async function listNotificationsApi(token: string) {
  return request<{ items: NotificationItem[]; unreadCount: number }>('', token)
}

export async function markNotificationReadApi(token: string, id: string) {
  return request<null>(`/${id}/read`, token, { method: 'POST' })
}

export async function markAllNotificationsReadApi(token: string) {
  return request<null>('/read-all', token, { method: 'POST' })
}
